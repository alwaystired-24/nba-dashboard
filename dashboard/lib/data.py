...
Migrated from SQLite to Postgres (Supabase) in Phase 1 (May 2026).
- _connect() returns a psycopg2 connection (caller wraps in `with closing(...)`)
- db_mtime() now queries MAX(last_attempt_utc) from etl_runs as a cache key
- All ? placeholders replaced with %s
- conn.execute(...) replaced with cursor pattern
- Row access is dict-style (RealDictCursor) — same ergonomics as SQLite Row
"""
...
# Load .env from repo root so DATABASE_URL is available locally.
# In Streamlit Cloud / GitHub Actions, DATABASE_URL is injected via env vars.
PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
load_dotenv(PROJECT_ROOT / ".env")

# Kept for backwards-compat with imports of DB_PATH elsewhere in the app
# (e.g. odds_data.py used to derive DB path from this). Now unused for the DB
# itself; callers should not reference it for new code.
DB_PATH = PROJECT_ROOT / "data" / "nba.db"

DATABASE_URL = os.getenv("DATABASE_URL")
if not DATABASE_URL:
    raise RuntimeError(
        "DATABASE_URL not set. Add it to .env locally, or to environment in deployment."
    )
...
def _connect():
    ...
    Use with `closing()` to ensure proper cleanup, e.g.:
        with closing(_connect()) as conn:
            ...

    RealDictCursor makes rows behave like dicts (`row["col_name"]`), matching
    the SQLite Row factory we used previously. pandas.read_sql_query also
    accepts this connection directly.
    """
    return psycopg2.connect(DATABASE_URL)


def _fetchone(conn, sql: str, params: tuple = ()) -> dict | None:
    """Convenience: execute SQL and return one row dict (or None).

    Uses RealDictCursor so rows behave like dicts (row["col_name"]),
    matching the SQLite Row factory ergonomics.
    """
...
def _fetchall(conn, sql: str, params: tuple = ()) -> list[dict]:
    """Convenience: execute SQL and return all rows as list of dicts.
...
def db_mtime() -> str:
    """Cache invalidation token. Changes whenever ETL writes a new row.

    Returns MAX(last_attempt_utc) from etl_runs as a string. Streamlit caches
    using this as the _mtime parameter — when this value changes (i.e. ETL
    ran), all data caches invalidate.

    Cached for 30s itself so we don't hit the DB on every Streamlit rerun.
    Returns empty string if the table is empty (fresh DB).
    """
    try:
        with closing(_connect()) as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute("SELECT MAX(last_attempt_utc) AS t FROM etl_runs")
                row = cur.fetchone()
                return str(row["t"]) if row and row["t"] else ""
    except Exception:
        return ""
...
def latest_loaded_date(_mtime: float = 0.0) -> str | None:
    ...
    with closing(_connect()) as conn:
        row = _fetchone(conn, "SELECT MAX(game_date) AS d FROM games")
    return row["d"] if row and row["d"] else None
    ...
    with closing(_connect()) as conn:
        return pd.read_sql_query(
            ...
            SELECT g.game_id, g.game_date, g.season_type,
                   CASE WHEN g.home_team_id = %s THEN 'H' ELSE 'A' END AS site,
                   CASE WHEN g.home_team_id = %s THEN g.away_team_id ELSE g.home_team_id END AS opp_id,
            ...
            JOIN team_box_traditional tbt
              ON tbt.game_id = g.game_id AND tbt.team_id = %s
            LEFT JOIN team_box_advanced tba
              ON tba.game_id = g.game_id AND tba.team_id = %s
            JOIN team_box_traditional opp_tbt
              ON opp_tbt.game_id = g.game_id AND opp_tbt.team_id != %s
            WHERE g.status = 'Final'
    ...
    with closing(_connect()) as conn:
        row = _fetchone(conn,
            ...
            WITH recent AS (
                ...
                WHERE pbt.player_id = %s AND g.status = 'Final' AND pbt.minutes > 0
                  {season_clause}
                ORDER BY g.game_date DESC
                {limit_clause}
            )
            ...
            JOIN player_box_traditional pbt
              ON pbt.game_id = r.game_id AND pbt.player_id = %s
            LEFT JOIN player_box_advanced pba
              ON pba.game_id = r.game_id AND pba.player_id = %s
            """,
...
def last_starting_lineup(team_id: int, _mtime: float = 0.0) -> pd.DataFrame:
    ...
    with closing(_connect()) as conn:
        ...
        return pd.read_sql_query(
            ...
            FROM player_box_traditional pbt
            JOIN players p ON p.player_id = pbt.player_id
            WHERE pbt.game_id = %s AND pbt.team_id = %s AND pbt.is_starter = 1
...
def team_minutes_forecast(team_id: int, _mtime: float = 0.0) -> pd.DataFrame:
    ...
    with closing(_connect()) as conn:
        ...
        last_game_players = pd.read_sql_query(
            ...
            FROM player_box_traditional pbt
            JOIN players p ON p.player_id = pbt.player_id
            WHERE pbt.game_id = %s AND pbt.team_id = %s
            ORDER BY pbt.minutes DESC
            """,
        ...
        l5_stats = pd.read_sql_query(
            ...
            SELECT pbt.player_id,
                   ...
                   COUNT(*) AS l5_gp
            FROM player_box_traditional pbt
            WHERE pbt.team_id = %s AND pbt.game_id IN ({placeholders})
